...
A **round** is one deal (deal, draws, discards, a win or an exhaustive draw), a **match** is the game the rounds sit inside (why: ADR-0023). `core/round.ts` is the engine that plays a hand — `RoundState`, `RoundOptions`, `RoundEvent`, `createRound`, `playRound`, `stepRound` — and `core/match.ts` is a small, standalone, pure module holding what persists _across_ rounds:
...
needs replaying, because replay puts every seat on manual and only the _starting_ algorithms of live
play matter.
...
board-wide, non-advanced, default off, reveals riichi threats live, and explicitly _not_ carved
...
does not, so the rule is simply _tiles on screen ⇒ badge_: your own seat always, every seat once
...
`showOpponentHands` is, not a jargon-gated extra. The per-discard feedback (an expanded log row's `log.folding.reason`
lines) names only the tier — it does not spell out why in a sentence of its own, trusting the
glossary entry to carry that instead. By design (ADR-0004): fold-only — no push
...
- **`wall` is the deal itself**, not a prefix consumed on the next draw: the whole deal in dealing order (4/4/4+1 per seat, ADR-0024), then the live draws, then the last 14 as the dead wall (five indicator-over-ura stacks then the four rinshan tiles, so the flipped indicator is the 9th of the 14 — ADR-0028). A short wall is completed at random from the copies it leaves. This is the **one codec in the repo that rejects rather than repairs** — `validateWall` sets `wallError` and empties `wall` instead, since a wall is positionally meaningful and a silent repair hands back a different board than the link claimed (contrast `parseTenhou`, which drops a malformed digit).
...
seat panel that _is_ a reader preference and stays persisted is `TableSettings.claims` — it answers
...
_that_ seat's hand there — face-down unless it is itself manual or hands are revealed, its 14th
...
outward, so the two seat _settings_ sit together at the corner and the two _hand reads_ (furiten,
...
overflows grows _inboard_ rather than pushing the wind off the felt. The wind is `Table`'s own styled
...
popover is upright wherever the seat is facing. It keeps `GlossaryTerm`'s _affordance_ though —
...
_outer_ square (the `relative` box the felt's own padding lives inside) — `display: contents` on the
...
The **session panel** is everything that is not the board or the hand: `panel` (the lab's own rankings and wall authoring — its one remaining consumer, so on every other trainer the panel simply _is_ the log menu) above `LogList`, which carries its own header now. One renderer, two placements: **docked** beside the board from `lg` up (`WIDE_QUERY`, read through `lib/useMediaQuery.ts` — `useSyncExternalStore`, so the first render already knows which shape it is drawing) and open by default, a **drawer** over the whole stage below that. Crossing the breakpoint resets it to that shape's own default, so a resize never leaves a drawer standing over tiles nobody asked to hide. Both halves of the panel are `flex-auto` rather than a fixed share: each is sized by its own content and only gives ground in proportion to it, so a long ranking gets the room an empty log is not using. Its width is a ramp, not one number — `clamp(20rem,26vw,28rem)`: 320px where it docks at all (1024px, where the board still wants every pixel), up to 448px on a desktop, where a 16:9 board is limited by its height and the width is spare. What the extra width buys is the log's own tiles (below). Its tiles draw at `0.6` — they are read, not played, and at hand size one ukeire list filled the column on its own — and both halves override **`--tile-w-base`**, not `--tile-w` alone, so a nested override composes with the 0.6 instead of ignoring it (`UkeireTiles` scales 0.8 off the base, and drew _larger_ than the tiles above it until it did).

`LogList` (`components/LogList.tsx`) is **the feedback surface**, not a transcript beside one. Tiles lead each row and the translated sentence follows at `text-xs` beneath them, muted — the river's own order of business, which the old sentence-first row had backwards. The tile line is the row's **whole width**: no ordinal column ahead of it (the ordinal leads the sentence line instead, where it is only ever read against `log.rewound`) and no action cluster beside it (rewind/share/copy/chevron sit at the end of the sentence line), which is what lets a thirteen-tile hand read on one line rather than wrapping to three. It fits by shrinking, not by wrapping: the list is an inline-size container and its tiles are `clamp(1.25rem, (100cqw - 2rem)/14, 0.6 x base)` — fourteen fit whatever column the panel currently is, floored where the tile art stops reading. A 2px **verdict spine** runs down the list's own left edge (`absolute inset-y-0 left-0`, and the panel hands the list `-mx-2` so it starts on the panel's border rather than 8px short of it), one segment per row, coloured from `LogEntry.severity` — green on `'ok'`, amber on `'warning'`, red on `'error'` — so read top to bottom it _is_ the session's accuracy record. Width carries it alongside colour, so it does not fail for a colour-blind reader. A row with **no** `severity` at all is a separate case from `'ok'` and deliberately does not fall back to it (`NO_VERDICT`, a neutral hairline): a rewind, a replayed discard, the tenpai note and every row the lab writes are not graded decisions, and a green bar beside one claims a verdict nobody gave. When the row names a better tile, `LogEntry.seam` says where in `tiles` that half begins and the two are separated by a hairline rule — two tiles either side of a seam is the diff, no arrow glyph needed. Rows carrying `LogEntry.detail` grow a chevron: expanding draws what the deleted feedback panels drew, one line per `LogDetail` (an i18n key plus params and tiles, never text — a language switch has to re-translate it, same reason the entry itself is a key; `formatLogDetail` in `features/i18n/formatLogEntry.ts` is its renderer, with the one `log.folding.reason` special case that composes a wind name from a raw seat number). `folding.equallySafe` is logged unconditionally and gated **at render** on `settings.folding.showEquallySafe`, exactly as `FoldFeedback` gated its own copy, so the toggle reaches rows already on the record. `log.dealt`/`log.dealtHand` draw as centred separators rather than rows — a deal really is a boundary — and break the spine. They carry **no tiles**: a hand is drawn on exactly one row, the one that grades it. Shanten used to draw its thirteen on both, which in a list read newest-first put the incoming hand's tiles directly above the _previous_ hand's verdict and read as the deal having replaced it; its dealt row keeps only what the graded row cannot offer — copy, rewind and share of a hand nobody has answered yet — plus the hand's number (`log.dealtHand` is "Hand {{hand}} dealt"), so a boundary in a reversed list says which hand it opens. The header holds the count, an `All | Mistakes` filter (`useState` local to the component: a way of reading this list right now, not a persisted preference) and Clear; ordinals are assigned **before** filtering, so a row keeps its number and `log.rewound`'s "Rewound to entry {{number}}" stays honest. Each action icon keeps a `size-3.5` glyph in a `size-6` box with a real ≥44px target from the `after:size-11` pseudo-element trick (`SeatPanel`).

`status` (the trainer's score/accuracy/clock lines) is not panel content (ADR-0026): a clock behind a drawer tap goes unread, so it floats instead as a small HUD inside the board area itself — the same box `noticeCompact` floats in, opposite it (bottom-left rather than top-centre, so the two never compete for the same strip), mirroring into the `short:` gutter the same way `noticeCompact` does. Always on screen at every viewport, panel open or shut. Not `pointer-events-none` as a whole: efficiency's ukeire line carries a live `GlossaryTerm` popover trigger, so only the wrapper is inert. Sharing the board itself is no longer a panel button either: every trainer's round-build effect now writes a `log.dealt`/`log.dealtHand` row the moment a board exists, and that row's own rewind/share buttons (`LogList.tsx`) are the one sharing surface a trainer has — the page-level `CopyLinkButton` pill is gone. That row is written per **board**, not per link: each hook's dedup ref has to key on whatever its own build effect keys on (efficiency and solo on `{ situation, restartCount }`, folding on the `RoundBoard` itself, scoring and shanten on their hand counter), since a local "New hand" is a new board under a URL that never moved and keying on the link alone left every board after the first unshareable. Not the `TableCore`, tempting as it looks: `useRound` rebuilds in its own effect, so the core a render captured is still the outgoing board by the time a consumer's effect reads it.

`onLogOpen` (the clock pause) is derived from **drawer** open, not panel open: a docked panel hides nothing, so it must not stop the clock. It is reported from an effect on that one derived flag rather than from the click handler, so every way it can change — a resize, Escape, the scrim — resumes exactly once and never lifts a pause the reader pressed themselves. The drawer is the _stage's_ own child, not the board area's, so it spans the whole page and covers the hand strip too — stopping at the board's bottom edge left the tiles it is meant to be read over showing underneath it — and it is a dialog like every other one in the app: `z-50` over the chrome row's `z-40`, its own scrim, dismissed by Escape or a press outside. It owns the top safe-area inset, since it reaches the top of the page.
