...
Two feedback densities, and which one you see is where the panel is: `notice` (full — `DiscardFeedback`/`FoldFeedback`, tile lists and ukeire counts included) is what the session panel holds, and `noticeCompact` (`features/table/Verdict.tsx` — one line, an icon, a colour and a short text, nothing else) floats over the board **only while that panel is shut**, falling back to `notice` when a caller has nothing compact to say. Open in either shape and the float does not render at all: the same verdict in two wordings on one screen reads as a difference rather than a repeat. In practice that lands compact on phones mid-drill and full on desktop, where the panel is docked. Severity is derived at display level from the existing grade/partial-credit, never a new grading concept: efficiency reads `TurnResult.grade` plus the shanten gap (`efficiencyVerdictSeverity`, `grade.ts` — green on `'ok'`, red only on an actual shanten regression, yellow for everything else `'error'`/`'warning'` catches); folding bands the same partial credit `useSessionStats` already averages into `averageQuality` (`foldingVerdictSeverity`, `useFoldingRound.ts` — green on `correct`, red below the halfway mark, yellow above it). The full breakdown stays one tap away in the log either way.

Tiles render as `<use>` references into a build-time SVG sprite (`src/assets/tiles/sprite.svg`, generated by `scripts/build-tile-sprite.mjs` from FluffyStuff assets, injected raw in `AppShell`). Tile size flows through the CSS var `--tile-w`; components scale locally by overriding it (e.g. `[--tile-w:calc(var(--tile-w-base)*0.8)]`). Tailwind 4; dark mode is a `dark` class on `<html>` toggled by `AppShell` from the persisted theme setting. `BoardStage` provides the chrome row, the settings dialog and the session panel — there is no separate page layout. Routing uses `basename: import.meta.env.BASE_URL` (GitHub Pages); pushes to `main` deploy via Actions, and the app is a PWA (`vite-plugin-pwa`, autoUpdate). Its icon (`public/favicon.svg`, hand-built and *not* produced by the sprite script) is a **square** `400x400`: a full-bleed felt-green field with the 1s tile centred at 78%. Both numbers are load-bearing — a home screen forces the icon square, so the tile's own 3:4 viewBox came out stretched, and 78% is what keeps the artwork inside the maskable safe zone the manifest now claims (`purpose: 'any maskable'`). No corner radius of its own: iOS's squircle and Android's adaptive shape both crop it, and a rounded source leaves white corners behind.
