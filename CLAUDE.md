...
`SeatButton` (`features/settings/SeatPanel.tsx`) is one icon per seat — not one table-wide panel —
sitting at that seat's own wind mark via `Table`'s `seatControl?: (seat: number) => ReactNode` prop,
opening a dialog scoped to that seat: "watch from here" (orientation, off via `orientable={false}`
