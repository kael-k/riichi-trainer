...
false, true, seed)` across 20 seeds produced exactly 1 red tile instead of the 3 `aka: true`
  promises, because the padding filter drops the _first_ occurrence of each id (which
  `completeWall` also always marks red) whenever `hand` holds a _plain_ copy of a red-eligible kind.
