...
export interface ScoringUrl {
  /** Explicit wall in draw order, exactly `urlCodec.ts`'s `Situation.wall`: a full
   *  wall, or a short prefix `createRound` pads at random. Empty means "generate a fresh hand". */
  wall: ParsedTile[]
  /** Set when `wall` failed `validateWall` — `wall` is then empty and a generated hand
...
export function decodeScoringUrl(params: URLSearchParams): ScoringUrl {
  ...
  // playWall — see `urlCodec.decodeSituation`'s identical gate. No global setting is
