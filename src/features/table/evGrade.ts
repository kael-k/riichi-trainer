 ...
 * the two-threshold band (`docs/model/limits.md#the-grading-band`), shared by every trainer that
 * reads it (folding's fold branch, efficiency's push branch). No React, no zustand: a pure module beside
...
/** ε₁ (still correct) and ε₂ (still partial credit), in points
 *  (`docs/model/limits.md#the-grading-band`). Beyond ε₂ grades wrong. */
...
/**
 * Provisional per-model defaults for **folding's fold branch**: one visible table, the
 * `TIER_SCORE` precedent — tune here, never scatter. Sized off
 ...
 * requirement** — re-fix these after the backtest
 * (`docs/model/limits.md#calibration`).
 *
 ...
 * the backtest, kept as two entries rather than one shared pair so either can move
 ...
 * total order — either serves, since both return the same `DiscardEv[]` shape), on the
 * two-threshold band.
 *
 ...
 * plus one bar per candidate, normalized on the ranking's own best entry — the grading UI must show
 * the band it graded against. `fraction` is computed here rather than in the
