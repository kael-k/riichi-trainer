...
Global settings (theme, tile size, yonma/sanma, language, tile numbers) also carry "translated yaku names": on by default, so yaku and win conditions read as "Pure straight" rather than "Ittsuu" — turn it off for the Japanese terms the scoring tables use. The row is hidden under Japanese and Chinese, where those already are the local names.
