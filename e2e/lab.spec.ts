...
/**
 * The lab's EV panel — `plans/EV-3` §9's screen, and the only place in the app that reads
...
test.describe('the lab', () => {
  ...
  test('prices a turn when asked, and shows the terms behind the number', async ({ page }) => {
    ...
    // below it means anything without all three
    await expect(page.getByText(/Formula model, playing for Points/i)).toBeVisible()
